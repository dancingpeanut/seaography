            ...
            match operation {
                ...
                FilterOperation::Contains => {
                    if let Some(value) = filter.get("contains") {
                        let value = types_map_helper
                            .async_graphql_value_to_sea_orm_value::<T>(column, &value)?;
                        let value_str: String = value.unwrap();
                        condition = condition.add(column.contains(&value_str));
                    }
                }
                FilterOperation::StartsWith => {
                    if let Some(value) = filter.get("starts_with") {
                        let value = types_map_helper
                            .async_graphql_value_to_sea_orm_value::<T>(column, &value)?;
                        let value_str: String = value.unwrap();
                        condition = condition.add(column.starts_with(&value_str));
                    }
                }
                FilterOperation::EndsWith => {
                    if let Some(value) = filter.get("ends_with") {
                        let value = types_map_helper
                            .async_graphql_value_to_sea_orm_value::<T>(column, &value)?;
                        let value_str: String = value.unwrap();
                        condition = condition.add(column.ends_with(&value_str));
                    }
                }
                FilterOperation::Like => {
                    if let Some(value) = filter.get("like") {
                        let value = types_map_helper
                            .async_graphql_value_to_sea_orm_value::<T>(column, &value)?;
                        let value_str: String = value.unwrap();
                        condition = condition.add(column.like(&value_str));
                    }
                }
                FilterOperation::NotLike => {
                    if let Some(value) = filter.get("not_like") {
                        let value = types_map_helper
                            .async_graphql_value_to_sea_orm_value::<T>(column, &value)?;
                        let value_str: String = value.unwrap();
                        condition = condition.add(column.not_like(&value_str));
                    }
                }
                FilterOperation::Between => {
